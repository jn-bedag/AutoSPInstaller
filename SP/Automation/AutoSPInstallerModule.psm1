...
Function AssignCert ($SSLHostHeader, $SSLPort, $SSLSiteName)
{
    ...
    If (!$cert)
    {
        Write-Error -ForegroundColor White "   - None found."
    }
    else
    {
        ...
        if (":$($SSLPort):$($SSLHostHeader)" -in (Get-IISSiteBinding -Name $SSLSiteName).bindingInformation)
        {
            Write-Host -ForegroundColor White " - Removing SharePoint default binding :$($SSLPort):$($SSLHostHeader)"
            Remove-IISSiteBinding -Name $SSLSiteName -BindingInformation ":$($SSLPort):$($SSLHostHeader)" -protocol "https" -Confirm:$false
        }


        if ("*:$($SSLPort):$($SSLHostHeader)" -in (Get-IISSiteBinding -Name $SSLSiteName).bindingInformation)
        {
            Write-Host -ForegroundColor White " - SSL binding *:$($SSLPort):$($SSLHostHeader) already present"
        }
        else
        {
            Write-Host -ForegroundColor White " - Creating binding and assigning certificate `"$certSubjectName`" to site $SSLHostHeader"
            New-IISSiteBinding -Name $SSLSiteName -BindingInformation "*:$($SSLPort):$($SSLHostHeader)" -CertificateThumbPrint $certThumbprint -CertStoreLocation "Cert:\LocalMachine\My" -Protocol https -SslFlag 1
            #TODO: bugfix to set-item, not new-item
        ...
        if ($SSLPort -eq "443"){
            ...
            if ("*:80:$($SSLHostHeader)" -in (Get-IISSiteBinding -Name $SSLSiteName).bindingInformation)
            {
                Write-Host -ForegroundColor White " - http binding already present"
            }
            else
            {
                Write-Host -ForegroundColor White " - Creating new http binding"
                New-IISSiteBinding -Name $SSLSiteName -BindingInformation "*:80:$($SSLHostHeader)" -Protocol http
            }
            
            #Check for AAM
            if ("http://$($SSLHostHeader)" -in $(Get-SPAlternateURL -WebApplication $SSLSiteName).IncomingUrl){
                Write-Host -ForegroundColor White "   - Internal AAM http://$($SSLHostHeader) already exist"
            }
            else
            {
